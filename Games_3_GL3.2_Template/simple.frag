...
vec3 lightUp(vec3 light_position, vec3 intensity, vec4 surf_colour){
    vec3 ambient = k_ambient * surf_colour.rgb * intensity;//actually, move ambient outside (independent of light sources)
...
void main( )
{
    ...
    //out_color = texture(t, frag_texCoord) * vec4(sum, 1.0f); //vec4(colour, 1.0);
